    ...
    public class SupportTicketController : BaseController
    {
        ...
        public string GetGridData(GridSettings grid)
        {
            try
            {
                ...
                return new SupportTicketBL().GetAllTickets(grid, oUser.UserId);
            }
            catch (Exception ex)
            {
                return ex.Message;
        ...
        public string GetAdvSearchResult(GridSettings grid, SearchTicketModel AdvSearch)
        {
            ...
            {
                User oUser = SiteUtility.GetCurrentUser();
                return new SupportTicketBL().GetAllTickets(grid, oUser.UserId, AdvSearch);
            }
            catch (Exception ex)
            {
                return "";
        ...
        public ActionResult View_Detail(Guid? id = null, string tab = "")
        {
            ...
            if (id != null)
            {
                Ticket oticket = new SupportTicketBL().GetByTicketId(id.Value);
        ...
        {
            Ticket oTicket = new SupportTicketBL().GetById(id);
            ViewBag.IsEditAllow = IsEditOtherAllow(oTicket);
        ...
        public ActionResult TicketDetail(Guid id)
        {
            ...
            ViewBag.IsOwnTicket = IsOwnTicket(oTicket);
            ViewBag.IsDeleteAllow = IsDeleteAllow(oTicket);
            ViewBag.IsEditAllow = IsEditOtherAllow(oTicket);
            ViewBag.IsReOpenAllow = IsReOpenAllow(oTicket);
            ViewBag.IsReassignAllow = IsReassignAllow(oTicket);
        ...
        public ActionResult Comments(Guid id)
        {
            ...
            ViewBag.lstComments = new CommentBL().GetOperatorComment(oTicket.TicketGuid);
            ViewBag.IsEditAllow = IsEditOtherAllow(oTicket);
        ...
        public ActionResult Save(Ticket oTicket, FormCollection collection)
        {
            try
            {
                ...
                if (id == 0)
                {
                    ...
                    new EmailTemplateBL().Creation_ByOperator_ToContact(oSupportTicket, ocompUser.Email, oUser.Name);
                    ...
                    if (collection["adImgs"] != null) //handle img for add only for edit it save directly.
                    {
                        ...
                        //if (collection["imgNames"] != null)
                        //{
                        //    #region Send Attachment mail

                        //    vw_SupportTicket VwTicket = new SupportTicketBL().GetTicketViewDetailById(oTicket.TicketId);
                        //    new AttachmentsBL().SendAttachmentMail(En_EmailKey.Creation_ByContact.ToString(), VwTicket, collection["imgNames"]);
                ...
                else
                {
                    ...
                    if (oldTicket.CurrentStatusId != oTicket.CurrentStatus && lstOpenStatusId.Contains(oTicket.CurrentStatus))
                    {
                        ...
                        new EmailTemplateBL().StatusChanged_ToOperator(oSupportTicket, oUser.Email,oUser.Name,oldTicket.StatusName);
                    }
                    ...
                    else if (!lstOpenStatusId.Contains(oTicket.CurrentStatus))
                    {
                        ...
                        new EmailTemplateBL().Closed_ToOperator(oSupportTicket, oUser.Email,oUser.Name,oldTicket.StatusName);
                    }
                    ...
                    {
                        //if (oldTicket.AssignToOperator == null)
                        //{
                        //    vw_SupportTicket oSupportTicket = new SupportTicketBL().GetTicketViewDetailById(oTicket.TicketId);
                        //    User optUser = new UserBL().GetById(oTicket.AssignToOperator.Value);
                        //    new EmailTemplateBL().Creation_ByOperator_ToOperator(oSupportTicket, optUser.Email, optUser.Name,false);
                        //}
                        //else
                        //{
                            vw_SupportTicket oSupportTicket = new SupportTicketBL().GetTicketViewDetailById(oTicket.TicketId);
                    ...
                    {
                        //if (oldTicket.AssignToSupportTeam == null)
                        //{
                        //    List<int> UserIds = new SupportTeamMembersBL().GetByTeamId(oTicket.AssignToSupportTeam.Value).Select(p => p.UserId).ToList();
                        //    vw_SupportTicket oSupportTicket = new SupportTicketBL().GetTicketViewDetailById(oTicket.TicketId);
                        //    new EmailTemplateBL().Creation_ByOperator_ToTeam(oSupportTicket, UserIds,false);
                        //}
                        //else
                        //{
                            List<int> UserIds = new SupportTeamMembersBL().GetByTeamId(oTicket.AssignToSupportTeam.Value).Select(p => p.UserId).ToList();
        ...
        public bool IsDeleteAllow(Ticket oTicket)
        {
            if (IsOwnTicket(oTicket))
            {
                if (SiteUtility.IsAllow((int)En_Permission.DeleteTickets))
                    return true;
            }
            else
            {
        ...
        public bool IsReassignAllow(Ticket oTicket)
        {
            if (IsOwnTicket(oTicket))
            {
        ...
        public ActionResult AddFiles()
        {
            ...
            string uploadDirectory = Server.MapPath(Constants.TicketImgUploadPath + "/" + oTicket.TicketId);
            ...
            foreach (string fileName in Request.Files)
            {
                ...
                if (file != null && file.ContentLength > 0)
                {
                    ...
                    //save to db
                    Attachment oAttachment = new Attachment();
                    oAttachment.LinkId = int.Parse(oTicket.TicketId.ToString());
                    oAttachment.FileName = file.FileName;
                    oAttachment.FilePath = string.Format("{0}\\{1}", oTicket.TicketId, fileId);
                    oAttachment.LinkType = (int)En_LinkType.SupportTicket;
            ...
            if (FileNames != "")
            {
                ...
                vw_SupportTicket VwTicket = new SupportTicketBL().GetTicketViewDetailById(Convert.ToInt32(oTicket.TicketId));
                string link = Utility.CommonFunction.GetSite_URL() + "/Account/SupportTicket/View_Detail?id=" + oTicket.TicketGuid + "&tab=Attachments";
        ...
        public JsonResult SaveActivity(TicketActivity oTicketActivity)
        {
            try
            {
                ...
                return Json(new { success = true, message = CommonMsg.Success(EntityNames.TicketActivity, Add_Flg == true ? En_CRUD.Insert : En_CRUD.Update) });
            }
            catch (Exception ex)
            {
                return Json(new { success = false, message = CommonMsg.Error() });
